<x-layouts.guest :title="__('Sign in to your account')">
    <div class="mb-8">
        <h1 class="text-2xl font-semibold text-slate-900">{{ __('Sign in to your account') }}</h1>
        <p class="mt-1 text-sm text-slate-500">{{ __('Use your email and password to login') }}</p>
    </div>
    ...
    <form class="space-y-6" method="POST" action="{{ route('login') }}">
        ...
        <div>
            ...
            <div class="relative">
                ...
                <div class="absolute inset-y-0 right-0 pr-3 flex items-center cursor-pointer" onclick="togglePassword()">
                    <i id="toggleIcon" class="fas fa-eye text-slate-400 hover:text-slate-600"></i>
                </div>
            </div>
        ...
        <button type="submit" class="btn-primary w-full py-2.5">{{ __('Login') }}</button>
