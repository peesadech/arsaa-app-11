@php
    ...
    $inputClass = 'form-input rounded-lg';
...
<form action="{{ $isEdit ? route('admin.students.update', $student->id) : route('admin.students.store') }}" method="POST" enctype="multipart/form-data">
    ...
    @endphp
    <div class="flex items-center gap-1.5 overflow-x-auto mb-6 pb-1">
        @foreach($formTabs as $key => $tab)
        <button type="button" data-tab-btn="{{ $key }}" onclick="showTab('{{ $key }}')"
                class="px-4 py-2 rounded-lg text-xs font-semibold whitespace-nowrap transition border border-slate-200 bg-white text-slate-600 hover:bg-slate-50">
            ...
            <div class="space-y-5">
                <div class="flex flex-col sm:flex-row gap-6">
                    ...
                    <div class="flex flex-col items-center gap-3">
                        <div class="w-32 h-40 rounded-2xl overflow-hidden bg-slate-100 border border-slate-200">
                            <img id="photoPreview" src="{{ $isEdit && $student->image_path ? asset($student->image_path) : 'https://ui-avatars.com/api/?name=' . urlencode($isEdit ? $student->name_th : '?') . '&color=2563eb&background=eff6ff&size=256' }}" class="w-full h-full object-cover" alt="">
                        </div>
                        <label class="btn-secondary cursor-pointer text-xs">
                            <x-icon name="upload" class="h-4 w-4" /> {{ __('Upload Photo') }}
                    ...
                    <div class="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="{{ $labelClass }}">{{ __('Student Code') }} <span class="text-slate-400 font-normal">({{ __('leave blank to auto-generate') }})</span></label>
                        ...
                        <div>
                            <label class="{{ $labelClass }}">{{ __('Birth Date') }} <span id="ageDisplay" class="text-brand-500 font-bold"></span></label>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div>
                        ...
                        <select name="race_id" class="form-select rounded-lg">
                            <option value="">-</option>
                    ...
                    <div>
                        ...
                        <select name="nationality_id" class="form-select rounded-lg">
                            <option value="">-</option>
                            @foreach($nationalities as $opt)
                    ...
                    <div>
                        ...
                        <select name="religion_id" class="form-select rounded-lg">
                            <option value="">-</option>
                    ...
                    <div>
                        ...
                        <select name="blood_type_id" class="form-select rounded-lg">
                            <option value="">-</option>
    ...
    <div data-tab-panel="address" class="hidden space-y-6">
        ...
        <x-card data-address-block="{{ $type }}">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-sm font-semibold text-slate-700 uppercase tracking-wide">{{ $title }}</h2>
                @if($type === 'registered')
                <button type="button" onclick="copyAddress()" class="btn-secondary text-xs"><x-icon name="clipboard" class="h-4 w-4" /> {{ __('Same as current address') }}</button>
                @endif
            </div>
            <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <select name="addresses[{{ $type }}][province_id]" class="form-select rounded-lg" data-addr-field="province_id">
                        <option value="">-</option>
    ...
    <div data-tab-panel="guardians" class="hidden">
        <x-card>
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-sm font-semibold text-slate-700 uppercase tracking-wide">{{ __('Guardians') }} <span class="normal-case font-normal text-slate-400">({{ __('select one as primary guardian') }})</span></h2>
                <button type="button" onclick="addGuardian()" class="btn-secondary text-xs"><x-icon name="plus" class="h-4 w-4" /> {{ __('Add Guardian') }}</button>
            </div>
    ...
    <div data-tab-panel="education" class="hidden">
        <x-card>
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-sm font-semibold text-slate-700 uppercase tracking-wide">{{ __('Previous Education (Chinese)') }}</h2>
                <button type="button" onclick="addEducation()" class="btn-secondary text-xs"><x-icon name="plus" class="h-4 w-4" /> {{ __('Add') }}</button>
            </div>
    ...
    <div data-tab-panel="documents" class="hidden">
        <x-card>
            <h2 class="text-sm font-semibold text-slate-700 uppercase tracking-wide mb-4">{{ __('Application Documents Checklist') }}</h2>
                ...
                <table class="w-full" style="min-width:650px">
                    ...
                    @foreach($documentTypes as $docType)
                        ...
                        <tr class="border-b border-slate-50">
                            ...
                            <td class="py-3 pr-3 text-sm text-slate-700">{{ $docType->name_th }}</td>
                            <td class="py-3 pr-3">
                                <input type="file" name="document_files[{{ $docType->id }}]" accept=".jpg,.jpeg,.png,.pdf"
                                       class="block w-full text-xs text-slate-500 file:mr-2 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-semibold file:bg-brand-50 file:text-brand-600 hover:file:bg-brand-100 cursor-pointer">
                                @if($doc?->file_path)
                                ...
                                <a href="{{ asset($doc->file_path) }}" target="_blank" class="inline-block mt-1">
                                    <img src="{{ asset($doc->file_path) }}" alt="{{ $docType->name_th }}" class="w-16 h-16 object-cover rounded-lg border border-slate-200 hover:opacity-80">
                                </a>
                                @else
                                <a href="{{ asset($doc->file_path) }}" target="_blank" class="text-[11px] text-brand-600 hover:underline"><i class="fas fa-paperclip mr-1"></i>{{ __('View attached file') }}</a>
                                @endif
                                @endif
                            </td>
                            <td class="py-3">
    ...
    <div data-tab-panel="history" class="hidden space-y-6">
        <x-card>
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-sm font-semibold text-slate-700 uppercase tracking-wide">{{ __('Classroom History') }}</h2>
                <a href="{{ route('admin.student-enrollments.index') }}" class="btn-secondary text-xs"><x-icon name="door" class="h-4 w-4" /> {{ __('Manage Enrollment') }}</a>
            </div>
            @if($student->enrollments->isEmpty())
            <p class="text-sm text-slate-400">{{ __('No enrollment history') }}</p>
            @else
            <div class="space-y-2">
                @foreach($student->enrollments->sortByDesc('id') as $enrollment)
                <div class="flex flex-wrap items-center gap-2 p-3 bg-slate-50 rounded-xl text-xs text-slate-600">
                    <span class="font-semibold text-slate-800">{{ __('Academic Year') }} {{ $enrollment->academicYear->year ?? '?' }} / {{ __('Semester') }} {{ $enrollment->semester->semester_number ?? '?' }}</span>
                    <span>{{ $enrollment->grade->name_th ?? '' }} / {{ $enrollment->classroom->name ?? '' }}</span>
                    @php
                        $enrollColors = ['enrolled' => 'bg-emerald-50 text-emerald-600', 'moved' => 'bg-amber-50 text-amber-600', 'left' => 'bg-red-50 text-red-600'];
                        $enrollLabels = ['enrolled' => __('Enrolled'), 'moved' => __('Moved'), 'left' => __('Left')];
                    @endphp
                    <span class="px-2 py-0.5 rounded-lg {{ $enrollColors[$enrollment->status] ?? 'bg-slate-100 text-slate-500' }} text-[10px] font-bold uppercase">{{ $enrollLabels[$enrollment->status] ?? $enrollment->status }}</span>
                    @if($enrollment->enrolled_at)<span class="text-slate-400">{{ $enrollment->enrolled_at->format('d/m/Y') }}</span>@endif
                    @if($enrollment->note)<span class="italic text-slate-400">— {{ $enrollment->note }}</span>@endif
                </div>
                @endforeach
            </div>
            @endif
        ...
        <x-card>
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-sm font-semibold text-slate-700 uppercase tracking-wide">{{ __('Academic Results') }}</h2>
                <a href="{{ route('admin.student-reports.transcript', $student->id) }}" target="_blank" class="btn-secondary text-xs"><x-icon name="clipboard" class="h-4 w-4" /> {{ __('Transcript') }}</a>
            </div>
            @if($student->scores->isEmpty())
            <p class="text-sm text-slate-400">{{ __('No scores recorded yet') }}</p>
            @else
            <div class="overflow-x-auto">
                <table class="w-full" style="min-width:600px">
                    ...
                    @foreach($student->scores as $score)
                        <tr class="border-b border-slate-50 text-xs text-slate-600">
                            ...
                            <td class="py-2 pr-3 text-center font-bold text-brand-600">{{ $score->grade ?? '-' }}</td>
                            <td class="py-2 text-center">
                                ...
                                <span class="px-2 py-0.5 rounded-lg bg-red-50 text-red-600 text-[10px] font-bold uppercase">{{ __('Fail') }}</span>
                                @else - @endif
                            </td>
                        </tr>
                    @endforeach
    ...
    <div class="mt-6 flex justify-end gap-2">
        <x-button variant="secondary" :href="route('admin.students.index')">{{ __('Cancel') }}</x-button>
...
function showTab(key) {
    ...
    document.querySelectorAll('[data-tab-btn]').forEach(b => {
        ...
        b.className = 'px-4 py-2 rounded-lg text-xs font-semibold whitespace-nowrap transition border ' +
            (active
...
const SELECT_CLASS = 'form-select rounded-lg';
...
function addGuardian(data = {}) {
    ...
    div.innerHTML = `
        <div class="flex items-center justify-between">
            <label class="flex items-center gap-2 cursor-pointer text-xs font-semibold text-brand-600">
                <input type="radio" name="primary_guardian" value="${i}" ${data.is_primary ? 'checked' : ''} class="h-4 w-4 text-brand-600 focus:ring-brand-500">
                {{ __('Primary guardian') }}
            </label>
            <button type="button" onclick="this.closest('[data-guardian]').remove()" class="text-red-400 hover:text-red-600 text-xs font-semibold"><i class="fas fa-trash-alt mr-1"></i>{{ __('Remove') }}</button>
        </div>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div><label class="${LABEL_CLASS}">{{ __('Guardian Type') }}</label><select name="guardians[${i}][guardian_type_id]" class="${SELECT_CLASS}">${optionHtml(GUARDIAN_TYPES, data.guardian_type_id)}</select></div>
            ...
            <div><label class="${LABEL_CLASS}">{{ __('Race') }}</label><select name="guardians[${i}][race_id]" class="${SELECT_CLASS}">${optionHtml(NATIONALITIES, data.race_id)}</select></div>
            <div><label class="${LABEL_CLASS}">{{ __('Nationality') }}</label><select name="guardians[${i}][nationality_id]" class="${SELECT_CLASS}">${optionHtml(NATIONALITIES, data.nationality_id)}</select></div>
            <div><label class="${LABEL_CLASS}">{{ __('Religion') }}</label><select name="guardians[${i}][religion_id]" class="${SELECT_CLASS}">${optionHtml(RELIGIONS, data.religion_id)}</select></div>
            <div><label class="${LABEL_CLASS}">{{ __('Living Status') }}</label><select name="guardians[${i}][living_status]" class="${SELECT_CLASS}">${optionHtml(LIVING_STATUSES, data.living_status)}</select></div>
...
function addEducation(data = {}) {
    ...
    div.innerHTML = `
        <div class="flex justify-end mb-2">
            <button type="button" onclick="this.closest('[data-education]').remove()" class="text-red-400 hover:text-red-600 text-xs font-semibold"><i class="fas fa-trash-alt mr-1"></i>{{ __('Remove') }}</button>
        </div>
