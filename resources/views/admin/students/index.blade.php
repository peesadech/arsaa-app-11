...
@push('styles')
    ...
    <style>
        ...
        .dt-buttons .btn {
            border-radius: 12px !important; font-size: 0.75rem !important; font-weight: 800 !important; text-transform: uppercase !important;
            letter-spacing: 0.05em !important; padding: 0.6rem 1.2rem !important; border: 1px solid #f3f4f6 !important;
            background: white !important; color: #6b7280 !important; transition: all 0.2s !important; box-shadow: none !important;
        }
        .dark .dt-buttons .btn { background: #242526 !important; border-color: #3a3b3c !important; color: #b0b3b8 !important; }
        .dt-buttons .btn:hover { border-color: #6366f1 !important; color: #6366f1 !important; }
    </style>
@endpush
    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-10 space-y-4 md:space-y-0">
            <div class="flex items-center space-x-6">
                ...
                <div>
                    <h1 class="text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight">{{ __('Student Management') }}</h1>
                    <p class="text-sm text-gray-500 dark:text-gray-400 font-medium mt-1 px-1">{{ __('Manage student records, guardians and enrollments') }}</p>
                </div>
            </div>

            <div class="flex flex-wrap gap-2">
                <a href="{{ route('admin.student-master.index') }}" class="btn-app"><i class="fas fa-database text-[10px]"></i> {{ __('Student Master Data') }}</a>
                <a href="{{ route('admin.student-reports.index') }}" class="btn-app"><i class="fas fa-chart-bar text-[10px]"></i> {{ __('Reports') }}</a>
                <a href="{{ route('admin.students.create') }}"
                   class="inline-flex items-center px-6 py-3 border border-transparent text-base font-bold rounded-2xl shadow-lg text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none transition-all duration-200 transform hover:-translate-y-0.5 active:scale-95">
                    <i class="fas fa-user-graduate mr-2 opacity-75"></i>
                    {{ __('New Student') }}
                </a>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-[#242526] rounded-[2.5rem] shadow-xl shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-[#3a3b3c] overflow-hidden transition-all duration-300">
            <div class="p-6 sm:p-8">
                    ...
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                <p class="text-[10px] text-gray-400 font-bold uppercase tracking-tight">{{ __('Filter by term, classroom and status') }}</p>
                            </div>
                        </div>

                        <div class="flex flex-wrap items-center gap-3">
                            <select id="yearFilter" class="appearance-none w-full md:w-36 pl-4 pr-8 py-2.5 bg-white dark:bg-[#242526] border-2 border-gray-100 dark:border-[#3a3b3c] rounded-xl text-xs font-bold text-gray-600 dark:text-gray-400 focus:outline-none focus:border-indigo-500 transition-all cursor-pointer">
                                <option value="">{{ __('All Years') }}</option>
                                @foreach($academicYears as $y)
                                <option value="{{ $y->id }}">{{ $y->year }}</option>
                                @endforeach
                            </select>
                            <select id="semesterFilter" class="appearance-none w-full md:w-32 pl-4 pr-8 py-2.5 bg-white dark:bg-[#242526] border-2 border-gray-100 dark:border-[#3a3b3c] rounded-xl text-xs font-bold text-gray-600 dark:text-gray-400 focus:outline-none focus:border-indigo-500 transition-all cursor-pointer">
                                <option value="">{{ __('All Semesters') }}</option>
                                @foreach($semesters as $s)
                                <option value="{{ $s->id }}">{{ __('Semester') }} {{ $s->semester_number }}</option>
                                @endforeach
                            </select>
                            <select id="classroomFilter" class="appearance-none w-full md:w-36 pl-4 pr-8 py-2.5 bg-white dark:bg-[#242526] border-2 border-gray-100 dark:border-[#3a3b3c] rounded-xl text-xs font-bold text-gray-600 dark:text-gray-400 focus:outline-none focus:border-indigo-500 transition-all cursor-pointer">
                                <option value="">{{ __('All Classrooms') }}</option>
                                @foreach($classrooms as $c)
                                <option value="{{ $c->id }}">{{ $c->name }}</option>
                                @endforeach
                            </select>
                            <select id="statusFilter" class="appearance-none w-full md:w-36 pl-4 pr-8 py-2.5 bg-white dark:bg-[#242526] border-2 border-gray-100 dark:border-[#3a3b3c] rounded-xl text-xs font-bold text-gray-600 dark:text-gray-400 focus:outline-none focus:border-indigo-500 transition-all cursor-pointer">
                ...
                <div class="overflow-x-auto lg:overflow-visible">
                    <table id="studentsTable" class="w-full text-left border-collapse whitespace-nowrap lg:whitespace-normal">
                        <thead>
                            <tr class="bg-gray-50/50 dark:bg-[#18191a]/30 border-b border-gray-100 dark:border-[#3a3b3c]/50">
                                <th class="px-6 py-4 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">{{ __('Photo') }}</th>
                                <th class="px-6 py-4 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">{{ __('Student Code') }}</th>
                                ...
                                <th class="px-6 py-4 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">{{ __('Mobile') }}</th>
            ...
            <div class="px-8 py-5 bg-gray-50/50 dark:bg-[#18191a]/30 border-t border-gray-100 dark:border-[#3a3b3c]/50 flex items-center justify-between text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">
                ...
                <span class="flex items-center"><i class="fas fa-user-graduate mr-2"></i> {{ __('Administrative Control') }}</span>
            </div>
        </div>
    </div>
</div>
                ...
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-6 sm:text-left">
                        ...
                        <div class="mt-2 text-sm text-gray-500 dark:text-gray-400 leading-relaxed">
                            {{ __('Are you sure you want to permanently remove') }} <span id="studentNameToDelete" class="font-bold text-gray-900 dark:text-white px-1.5 py-0.5 bg-gray-100 dark:bg-zinc-700 rounded-lg"></span>? {{ __('This action cannot be undone.') }}
                        </div>
                    </div>
                </div>
            </div>
...
@push('scripts')
    ...
    <script>
        const LANG_SEARCH_STUDENTS = {!! json_encode(__('Search by code, name, phone...')) !!};
        ...
        $(document).ready(function() {
            ...
            $('#studentsTable').DataTable({
                ...
                ajax: {
                    ...
                    data: function (d) {
                        ...
                        d.academic_year_id = $('#yearFilter').val();
                        d.semester_id = $('#semesterFilter').val();
                        d.classroom_id = $('#classroomFilter').val();
                    }
                },
                columns: [
                    { data: 'avatar', name: 'avatar', orderable: false, searchable: false, className: 'px-6 py-4' },
                    { data: 'student_code', name: 'student_code', className: 'px-6 py-4 font-bold text-xs' },
                    { data: 'name_display', name: 'name_th', className: 'px-6 py-4' },
                    { data: 'current_classroom', name: 'current_classroom', orderable: false, searchable: false, className: 'px-6 py-4' },
                    { data: 'mobile', name: 'mobile', className: 'px-6 py-4 text-xs', render: d => d || '-' },
                    { data: 'status_badge', name: 'status', className: 'px-6 py-4' },
                    { data: 'action', name: 'action', orderable: false, searchable: false, className: 'px-6 py-4 text-right' }
                ],
                language: {
                    search: "",
                    searchPlaceholder: LANG_SEARCH_STUDENTS,
            ...
            $('#statusFilter, #yearFilter, #semesterFilter, #classroomFilter').on('change', function() {
                $('#studentsTable').DataTable().draw();
            });
        });

        function confirmDelete(id, name) {
            $('#studentNameToDelete').text(name);
