<x-layouts.admin :header="__('Teachers')" :subheader="__('Manage teachers and their assigned courses')">
    <x-slot name="actions">
        <form action="{{ route('admin.teachers.create-accounts') }}" method="POST"
              onsubmit="return confirm('{{ __('Create login accounts for all teachers without one?') }}')">
            @csrf
            <x-button variant="secondary" type="submit" icon="user"
                      title="{{ __('Create login accounts for teachers that do not have one yet') }}">
                {{ __('Create missing accounts') }}
            </x-button>
        </form>
        <x-button icon="plus" :href="route('admin.teachers.create')">{{ __('New Teacher') }}</x-button>
    </x-slot>

    <div x-data="{ deleteTarget: null }" x-on:open-delete.window="deleteTarget = $event.detail">
        <x-card padded="false">
            <x-data-table
                :endpoint="route('admin.teachers.index')"
                :columns="[
                    ['key' => 'name', 'label' => __('Teacher Details'), 'sortable' => true],
                    ['key' => null, 'label' => __('Courses'), 'sortable' => false],
                    ['key' => null, 'label' => __('Account'), 'sortable' => false],
        ...
        <div x-show="deleteTarget !== null" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" x-transition.opacity>
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
                ...
                <div class="mt-6 flex justify-end gap-2">
                    ...
                    <form method="POST" x-bind:action="`{{ url('admin/teachers') }}/${deleteTarget?.id}`">
                        @csrf @method('DELETE')
