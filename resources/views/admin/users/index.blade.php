<x-layouts.admin :header="__('User Management')" :subheader="__('Manage system users, roles, and profiles')">
    <x-slot name="actions">
        <x-button icon="plus" :href="route('admin.users.create')">{{ __('New User') }}</x-button>
    </x-slot>
        ...
        <x-card padded="false">
            <x-data-table
                ...
                :columns="[
                    ...
                    ['key' => null, 'label' => __('Roles'), 'sortable' => false],
            ...
            >
                <x-slot name="filters">
                    <select x-model="state.role" @change="changeFilter()"
        ...
        <div x-show="deleteTarget !== null" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" x-transition.opacity>
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
                ...
                <div class="mt-6 flex justify-end gap-2">
                    ...
                    <form method="POST" x-bind:action="`{{ url('admin/users') }}/${deleteTarget?.id}`">
                        @csrf @method('DELETE')
