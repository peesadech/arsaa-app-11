<x-layouts.admin :header="__('Conduct Criteria')" :subheader="__('Conduct assessment items (manners, dress, service, discipline)')">
    <x-slot name="actions">
        <x-button icon="plus" :href="route('admin.conduct-criteria.create')">{{ __('New Item') }}</x-button>
    </x-slot>

    <div x-data="{ deleteTarget: null }" x-on:open-delete.window="deleteTarget = $event.detail">
        <x-card padded="false">
            <x-data-table
                :endpoint="route('admin.conduct-criteria.index')"
                :columns="[
                    ['key' => 'name', 'label' => __('Name'), 'sortable' => true],
                    ['key' => 'max_score', 'label' => __('Full Score'), 'sortable' => false, 'align' => 'right'],
            ...
            >
                <x-slot name="filters">
                    <select x-model="state.is_active" @change="changeFilter()" class="form-select rounded-lg w-full sm:w-48">
                        <option value="">{{ __('All Status') }}</option>
                        <option value="1">{{ __('Active') }}</option>
                        <option value="0">{{ __('Inactive') }}</option>
                    </select>
                    <label class="relative block w-full sm:w-64">
                        <span class="absolute inset-y-0 left-0 flex items-center pl-3 text-slate-400 pointer-events-none"><x-icon name="search" class="h-4 w-4" /></span>
                        <input type="search" x-model="state.search" @input.debounce.350ms="changeFilter()" placeholder="{{ __('Search') }}..." class="form-input pl-9 rounded-lg w-full">
                    </label>
                </x-slot>
                <x-slot name="rows">
                    @include('admin.conduct-criteria._rows', ['items' => $items])
                </x-slot>
            </x-data-table>
        </x-card>
        ...
        <div x-show="deleteTarget !== null" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" x-transition.opacity>
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
                ...
                <div class="mt-6 flex justify-end gap-2">
                    ...
                    <form method="POST" x-bind:action="`{{ url('admin/conduct-criteria') }}/${deleteTarget?.id}`">
                        @csrf @method('DELETE')
