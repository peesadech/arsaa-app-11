    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-10 space-y-4 md:space-y-0">
            <div class="flex items-center space-x-6">
                ...
                <div>
                    <h1 class="text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight">{{ __('Academic Year Management') }}</h1>
                    <p class="text-sm text-gray-500 dark:text-gray-400 font-medium mt-1 px-1">{{ __('Manage system academic years') }}</p>
                </div>
            </div>
            ...
            <div>
                <a href="{{ route('admin.academic-years.create') }}" 
                   class="inline-flex items-center px-6 py-3 border border-transparent text-base font-bold rounded-2xl shadow-lg text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all duration-200 transform hover:-translate-y-0.5 active:scale-95">
                    <i class="fas fa-calendar-plus mr-2 opacity-75"></i>
        ...
        <div class="bg-white dark:bg-[#242526] rounded-[2.5rem] shadow-xl shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-[#3a3b3c] overflow-hidden transition-all duration-300 transform transition-all">
            <div class="p-6 sm:p-8">
                    ...
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                <p class="text-[10px] text-gray-400 font-bold uppercase tracking-tight">{{ __('Refine year list') }}</p>
                            </div>
                        </div>
                ...
                <div class="overflow-x-auto lg:overflow-visible">
                    <table id="yearsTable" class="w-full text-left border-collapse whitespace-nowrap lg:whitespace-normal">
                        <thead>
                            <tr class="bg-gray-50/50 dark:bg-[#18191a]/30 border-b border-gray-100 dark:border-[#3a3b3c]/50">
                                <th class="px-6 py-4 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest first:rounded-tl-2xl">{{ __('Academic Year') }}</th>
            ...
            <div class="px-8 py-5 bg-gray-50/50 dark:bg-[#18191a]/30 border-t border-gray-100 dark:border-[#3a3b3c]/50 flex items-center justify-between text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">
                <span>{{ __('Academic Year Management System') }}</span>
                <span class="flex items-center">
                    <i class="fas fa-calendar-check mr-2"></i> {{ __('Administrative Control') }}
                </span>
            </div>
        </div>
    </div>
                ...
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-6 sm:text-left">
                        ...
                        <div class="mt-2 text-sm text-gray-500 dark:text-gray-400 leading-relaxed">
                            {{ __('Are you sure you want to permanently remove') }} {{ __('Academic Year') }} <span id="itemNameToDelete" class="font-bold text-gray-900 dark:text-white px-1.5 py-0.5 bg-gray-100 dark:bg-zinc-700 rounded-lg"></span>? {{ __('This action cannot be undone.') }}
                        </div>
                    </div>
                </div>
            </div>
...
@push('scripts')
    ...
    <script>
        const LANG_SEARCH_YEARS = @json(__('Search years...'));
        ...
        $(document).ready(function() {
            ...
            $('#yearsTable').DataTable({
                ...
                ajax: {
                    url: "{{ route('admin.academic-years.data') }}",
                    data: function (d) {
                        d.status = $('#statusFilter').val();
                    }
                },
                columns: [
                    { data: 'year', name: 'year', className: 'px-6 py-4 font-bold text-gray-800 dark:text-gray-200' },
                ...
                language: {
                    search: "",
                    searchPlaceholder: LANG_SEARCH_YEARS,
