...
<x-layouts.admin :header="$educationLevel->name_th" :subheader="$educationLevel->name_en . ' — ' . __('Academic Year :year', ['year' => $academicYear->year]) . ' / ' . __('Semester Number') . ' ' . $semester->semester_number">
    <x-slot name="actions">
        <x-button variant="secondary" icon="arrow-left" :href="route('admin.yearly-schedule.index')">{{ __('Back') }}</x-button>
    </x-slot>
