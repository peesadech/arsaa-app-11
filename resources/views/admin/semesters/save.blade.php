...
@php
    ...
    $actionUrl = $isEdit ? route('admin.semesters.update', $semester->id) : route('admin.semesters.store');
    
    $title = $isEdit ? __('Edit Semester') : __('Create New Semester');
    $subtitle = $isEdit ? __('Update semester details') : __('Semester Registration');
    ...
    $cardTitle = $isEdit ? __('Modify Semester') : __('Semester Details');
    $cardDesc = $isEdit
        ? __('You are updating semester') . " #{$semester->id}."
        : __('Enter the semester number.');
    ...
    $btnText = $isEdit ? __('Save Changes') : __('Create Semester');
    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white dark:bg-[#242526] rounded-[2.5rem] shadow-xl shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-[#3a3b3c] overflow-hidden transform transition-all">
            ...
            <div class="p-8 sm:p-10">
                ...
                <!-- Form -->
                <form action="{{ $actionUrl }}" method="POST" class="space-y-6" id="semesterForm">
                    @csrf
                    @if($isEdit)
                        @method('PUT')
                    @endif
                    ...
                    <div class="space-y-2">
                        <label for="semester_number" class="block text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest px-1">
                            {{ __('Semester Number') }}
                        </label>
                        <div class="group relative">
                            ...
                            <input
                                ...
                                class="block w-full pl-10 pr-4 py-4 bg-gray-50 dark:bg-[#3a3b3c] border-2 border-transparent rounded-2xl text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-0 {{ $focusRing }} focus:bg-white dark:focus:bg-[#3a3b3c] transition-all duration-200 @error('semester_number') border-rose-300 bg-rose-50 dark:bg-rose-900/20 @enderror"
                                placeholder=""
                                value="{{ old('semester_number', $isEdit ? $semester->semester_number : '') }}"
                                required
                            />
                        </div>
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            <label class="relative group cursor-pointer">
                                <input type="radio" name="status" value="1" class="peer hidden" {{ old('status', $isEdit ? $semester->status : 1) == 1 ? 'checked' : '' }}>
                            ...
                            <!-- Not Active Status -->
                            <label class="relative group cursor-pointer">
                                <input type="radio" name="status" value="2" class="peer hidden" {{ old('status', $isEdit ? $semester->status : 1) == 2 ? 'checked' : '' }}>
