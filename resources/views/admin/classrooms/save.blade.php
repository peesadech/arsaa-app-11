...
@php
    ...
    $actionUrl = $isEdit ? route('admin.classrooms.update', $classroom->id) : route('admin.classrooms.store');

    $title = $isEdit ? __('Edit Classroom') : __('Create New Classroom');
    $subtitle = $isEdit ? __('Update classroom details') : __('Classroom Registration');
    ...
    $cardTitle = $isEdit ? __('Modify Classroom') : __('Classroom Details');
    $cardDesc = $isEdit
        ? __('You are updating classroom #:id. Ensure all details are correct.', ['id' => $classroom->id])
        : __('Define a new system classroom.');
    ...
    $btnText = $isEdit ? __('Save Changes') : __('Create Classroom');
    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white dark:bg-[#242526] rounded-[2.5rem] shadow-xl shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-[#3a3b3c] overflow-hidden transform transition-all">
            ...
            <div class="p-8 sm:p-10">
                ...
                <!-- Form -->
                <form action="{{ $actionUrl }}" method="POST" class="space-y-6" id="classroomForm">
                    @csrf
                    @if($isEdit)
                        @method('PUT')
                    @endif
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="group relative">
                                ...
                                <input
                                    ...
                                    placeholder="{{ __('e.g. 1/1') }}"
                                    value="{{ old('name', $isEdit ? $classroom->name : '') }}"
                                    required
                                />
                            </div>
                            @error('name')
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            <label class="relative group cursor-pointer">
                                <input type="radio" name="status" value="1" class="peer hidden" {{ old('status', $isEdit ? $classroom->status : 1) == 1 ? 'checked' : '' }}>
                            ...
                            <!-- Not Active Status -->
                            <label class="relative group cursor-pointer">
                                <input type="radio" name="status" value="2" class="peer hidden" {{ old('status', $isEdit ? $classroom->status : 1) == 2 ? 'checked' : '' }}>
