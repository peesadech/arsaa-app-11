<x-layouts.admin :header="__('Grading Scheme Management')" :subheader="__('Define how scores are converted to results')">
    <x-slot name="actions">
        <x-button icon="plus" :href="route('admin.grading-schemes.create')">{{ __('New Grading Scheme') }}</x-button>
    </x-slot>

    <div x-data="{ deleteTarget: null }" x-on:open-delete.window="deleteTarget = $event.detail">
        <x-card padded="false">
            <x-data-table
                :endpoint="route('admin.grading-schemes.index')"
                :columns="[
                    ['key' => 'name', 'label' => __('Scheme name'), 'sortable' => true],
                    ['key' => null, 'label' => __('Result type'), 'sortable' => false],
                ...
                <x-slot name="filters">
                    ...
                    <label class="relative block w-full sm:w-64">
                        ...
                        <input type="search" x-model="state.search" @input.debounce.350ms="changeFilter()"
                               placeholder="{{ __('Search grading schemes...') }}" class="form-input pl-9 rounded-lg w-full">
                    </label>
                </x-slot>

                <x-slot name="rows">
                    @include('admin.grading-schemes._rows', ['schemes' => $schemes])
                </x-slot>
            </x-data-table>
        </x-card>
        ...
        <div x-show="deleteTarget !== null" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" x-transition.opacity>
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
                ...
                <div class="mt-6 flex justify-end gap-2">
                    ...
                    <form method="POST" x-bind:action="`{{ url('admin/grading-schemes') }}/${deleteTarget?.id}`">
                        @csrf @method('DELETE')
