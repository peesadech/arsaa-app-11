<x-layouts.admin :header="__('Grades')" :subheader="__('Manage grades and their education levels')">
    <x-slot name="actions">
        <x-button icon="plus" :href="route('admin.grades.create')">{{ __('New Grade') }}</x-button>
    </x-slot>

    <div x-data="{ deleteTarget: null }" x-on:open-delete.window="deleteTarget = $event.detail">
        <x-card padded="false">
            <x-data-table
                :endpoint="route('admin.grades.index')"
                :columns="[
                    ['key' => 'name_en', 'label' => __('Name'), 'sortable' => true],
                    ['key' => null, 'label' => __('Education Level'), 'sortable' => false],
            ...
            >
                ...
                <x-slot name="rows">
                    @include('admin.grades._rows', ['grades' => $grades])
                </x-slot>
            </x-data-table>
        </x-card>
        ...
        <div x-show="deleteTarget !== null" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" x-transition.opacity>
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
                ...
                <div class="mt-6 flex justify-end gap-2">
                    ...
                    <form method="POST" x-bind:action="`{{ url('admin/grades') }}/${deleteTarget?.id}`">
                        @csrf @method('DELETE')
