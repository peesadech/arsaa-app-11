    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-10 space-y-4 md:space-y-0">
            <div class="flex items-center space-x-6">
                ...
                <div>
                    <h1 class="text-3xl font-extrabold text-gray-900 dark:text-white tracking-tight">{{ __('Subject Group Management') }}</h1>
                    <p class="text-sm text-gray-500 dark:text-gray-400 font-medium mt-1 px-1">{{ __('Manage subject groups and categories') }}</p>
                </div>
            </div>

            <div>
                <a href="{{ route('admin.subject-groups.create') }}"
        ...
        <div class="bg-white dark:bg-[#242526] rounded-[2.5rem] shadow-xl shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-[#3a3b3c] overflow-hidden transition-all duration-300 transform transition-all">
            <div class="p-6 sm:p-8">
                    ...
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                ...
                                <p class="text-[10px] text-gray-400 font-bold uppercase tracking-tight">{{ __('Refine subject group list') }}</p>
                            </div>
                        </div>
                ...
                <div class="overflow-x-auto lg:overflow-visible">
                    <table id="subjectGroupsTable" class="w-full text-left border-collapse whitespace-nowrap lg:whitespace-normal">
                        <thead>
            ...
            <div class="px-8 py-5 bg-gray-50/50 dark:bg-[#18191a]/30 border-t border-gray-100 dark:border-[#3a3b3c]/50 flex items-center justify-between text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest">
                <span>{{ __('Subject Group Management System') }}</span>
                <span class="flex items-center">
                    <i class="fas fa-th-large mr-2"></i> {{ __('Administrative Control') }}
                </span>
            </div>
        </div>
    </div>
...
@push('scripts')
    ...
    <script>
        const LANG_SEARCH_SUBJECT_GROUPS = @json(__('Search subject groups...'));
        ...
        $(document).ready(function() {
            ...
            $('#subjectGroupsTable').DataTable({
                ...
                ajax: {
                    url: "{{ route('admin.subject-groups.data') }}",
                    data: function (d) {
                ...
                language: {
                    search: "",
                    searchPlaceholder: LANG_SEARCH_SUBJECT_GROUPS,
            ...
            $('#statusFilter').on('change', function() {
                $('#subjectGroupsTable').DataTable().draw();
            });
        });

        function confirmDelete(id, name) {
            $('#itemNameToDelete').text(name);
            $('#deleteForm').attr('action', '/admin/subject-groups/' + id);
