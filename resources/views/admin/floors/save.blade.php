...
@php
    $isEdit = isset($floor);
    $actionUrl = $isEdit ? route('admin.floors.update', $floor->id) : route('admin.floors.store');

    $title = $isEdit ? __('Edit Floor') : __('Create New Floor');
    $subtitle = $isEdit ? __('Update floor details') : __('Floor Registration');
    ...
    $cardTitle = $isEdit ? __('Modify Floor') : __('Floor Details');
    $cardDesc = $isEdit
        ? __('You are updating floor #:id. Ensure all details are correct.', ['id' => $floor->id])
        : __('Define a new floor and its descriptions.');
    ...
    $btnText = $isEdit ? __('Save Changes') : __('Create Floor');
    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white dark:bg-[#242526] rounded-[2.5rem] shadow-xl shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-[#3a3b3c] overflow-hidden transform transition-all">
            ...
            <div class="p-8 sm:p-10">
                ...
                <!-- Form -->
                <form action="{{ $actionUrl }}" method="POST" class="space-y-6" id="floorForm">
                    @csrf
                    @if($isEdit)
                        @method('PUT')
                    @endif
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="group relative">
                            ...
                            <select
                                id="building_id"
                                name="building_id"
                                class="block w-full pl-10 pr-4 py-4 bg-gray-50 dark:bg-[#3a3b3c] border-2 border-transparent rounded-2xl text-gray-900 dark:text-white focus:outline-none focus:ring-0 {{ $focusRing }} focus:bg-white dark:focus:bg-[#3a3b3c] transition-all duration-200 @error('building_id') border-rose-300 bg-rose-50 dark:bg-rose-900/20 @enderror"
                                required
                            >
                                <option value="">{{ __('Select Building') }}</option>
                                @foreach($buildings as $building)
                                    <option value="{{ $building->id }}" {{ old('building_id', $isEdit ? $floor->building_id : '') == $building->id ? 'selected' : '' }}>
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="group relative">
                                ...
                                <input
                                    ...
                                    placeholder="{{ __('e.g. ชั้น 1') }}"
                                    value="{{ old('name_th', $isEdit ? $floor->name_th : '') }}"
                                    required
                                />
                            </div>
                            @error('name_th')
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="group relative">
                                ...
                                <input
                                    ...
                                    value="{{ old('name_en', $isEdit ? $floor->name_en : '') }}"
                                    required
                                />
                            </div>
                            @error('name_en')
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="group relative">
                            ...
                            >{{ old('description', $isEdit ? $floor->description : '') }}</textarea>
                        </div>
                        @error('description')
                            <p class="text-[10px] font-bold text-rose-500 mt-1 px-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            <label class="relative group cursor-pointer">
                                <input type="radio" name="status" value="1" class="peer hidden" {{ old('status', $isEdit ? $floor->status : 1) == 1 ? 'checked' : '' }}>
                            ...
                            <!-- Not Active Status -->
                            <label class="relative group cursor-pointer">
                                <input type="radio" name="status" value="2" class="peer hidden" {{ old('status', $isEdit ? $floor->status : 1) == 2 ? 'checked' : '' }}>
