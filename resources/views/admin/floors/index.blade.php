<x-layouts.admin :header="__('Floors')" :subheader="__('Manage building floors')">
    <x-slot name="actions">
        <x-button icon="plus" :href="route('admin.floors.create')">{{ __('New Floor') }}</x-button>
    </x-slot>

    <div x-data="{ deleteTarget: null }" x-on:open-delete.window="deleteTarget = $event.detail">
        <x-card padded="false">
            <x-data-table
                :endpoint="route('admin.floors.index')"
                :columns="[
            ...
            >
                ...
                <x-slot name="rows">
                    @include('admin.floors._rows', ['floors' => $floors])
                </x-slot>
            </x-data-table>
        </x-card>
        ...
        <div x-show="deleteTarget !== null" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" x-transition.opacity>
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
                ...
                <div class="mt-6 flex justify-end gap-2">
                    ...
                    <form method="POST" x-bind:action="`{{ url('admin/floors') }}/${deleteTarget?.id}`">
                        @csrf @method('DELETE')
