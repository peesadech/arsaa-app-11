<x-layouts.admin :header="__('System Permissions')" :subheader="__('Manage and organize granular access controls')">
    <x-slot name="actions">
        <x-button icon="plus" :href="route('admin.permissions.create')">{{ __('New Permission') }}</x-button>
    </x-slot>
                ...
                <table id="permissionsTable" class="w-full text-left border-collapse whitespace-nowrap lg:whitespace-normal">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-100">
                            ...
                            <th class="px-4 sm:px-6 py-3 text-xs font-medium text-slate-500 uppercase tracking-wide">{{ __('Permission Name') }}</th>
                            <th class="px-4 sm:px-6 py-3 text-xs font-medium text-slate-500 uppercase tracking-wide">{{ __('Guard') }}</th>
            ...
            var table = $('#permissionsTable').DataTable({
                ...
                columns: [
                    ...
                    },
                    {
                        ...
                        name: 'guard_name',
                        render: function(data) {
                            return '<span class="px-2 py-1 rounded-md bg-slate-100 text-slate-500 text-[10px] font-bold uppercase tracking-widest">' + data + '</span>';
                        }
                    },
                    {
                        data: 'action',
