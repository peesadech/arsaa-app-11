<x-layouts.admin :header="__('Rooms')" :subheader="__('Manage room numbers and assigned courses')">
    <x-slot name="actions">
        <x-button icon="plus" :href="route('admin.rooms.create')">{{ __('New Room') }}</x-button>
    </x-slot>

    <div x-data="{ deleteTarget: null }" x-on:open-delete.window="deleteTarget = $event.detail">
        <x-card padded="false">
            <x-data-table
                :endpoint="route('admin.rooms.index')"
                :columns="[
                    ['key' => 'room_number', 'label' => __('Room Number'), 'sortable' => true],
                    ['key' => null, 'label' => __('Building'), 'sortable' => false],
                    ['key' => null, 'label' => __('Floor'), 'sortable' => false],
            ...
            >
                <x-slot name="filters">
                    <select x-model="state.building_id" @change="changeFilter()"
                ...
                <x-slot name="rows">
                    @include('admin.rooms._rows', ['rooms' => $rooms])
                </x-slot>
            </x-data-table>
        </x-card>
        ...
        <div x-show="deleteTarget !== null" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" x-transition.opacity>
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
                ...
                <div class="mt-6 flex justify-end gap-2">
                    ...
                    <form method="POST" x-bind:action="`{{ url('admin/rooms') }}/${deleteTarget?.id}`">
                        @csrf @method('DELETE')
