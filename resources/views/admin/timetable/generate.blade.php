...
<div class="min-h-screen bg-gray-50 dark:bg-[#18191a] py-10 px-4 sm:px-6 lg:px-8 transition-colors duration-300">
    <div class="max-w-3xl mx-auto">
        ...
        <div class="flex items-center space-x-4 mb-8">
            ...
            <div>
                <h1 class="text-2xl font-extrabold text-gray-900 dark:text-white tracking-tight">Generate ตารางเรียน</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 font-medium px-1">Genetic Algorithm Timetable Generator</p>
            </div>
        </div>
        ...
        <form action="{{ route('admin.timetable.generate.store') }}" method="POST">
            ...
            <div class="bg-white dark:bg-[#242526] rounded-[2rem] shadow-sm border border-gray-100 dark:border-[#3a3b3c] p-6 space-y-6">
                ...
                {{-- Algorithm Settings --}}
                <div>
                    <h3 class="text-sm font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wide mb-4">Algorithm Settings</h3>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Population Size</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">Max Generations</label>
                            <input type="number" name="max_generations" value="500" min="50" max="2000"
                                   class="w-full px-4 py-3 bg-gray-50 dark:bg-[#3a3b3c] border-2 border-transparent rounded-2xl text-gray-800 dark:text-white focus:border-indigo-500 focus:outline-none transition-all">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-1">จำนวน Solutions</label>
                ...
                {{-- Scope --}}
                <div>
                    <h3 class="text-sm font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wide mb-4">ขอบเขต (ไม่เลือก = ทั้งหมด)</h3>

                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-2">ระดับชั้น</label>
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-600 dark:text-gray-400 mb-2">ห้องเรียน</label>
                ...
                <div class="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 rounded-2xl text-blue-700 dark:text-blue-300 text-sm">
                    ...
                    ระบบจะ Generate ตารางเรียนโดยใช้ Genetic Algorithm เพื่อหาตารางที่ดีที่สุด
                    กระบวนการจะทำงานเบื้องหลังและอาจใช้เวลา 1-5 นาที
                </div>

                <div class="flex justify-end">
                    <button type="submit" class="btn-app px-8"
                            onclick="return confirm('ยืนยันการ Generate ตารางเรียน?')">
                        <i class="fas fa-play text-[10px]"></i> เริ่ม Generate
                    </button>
                </div>
            </div>
        </form>
