<x-layouts.admin :header="__('Room Schedule') . ' ' . $room->room_number . ' ' . ($room->building ? '('.$room->building->name_th.')' : '')">
    <x-slot name="actions">
                    ...
                    @foreach($teachingDays as $d)
                        ...
                        <td class="border border-slate-200 p-1">
                            <div class="p-2 rounded-xl border {{ $colors[$colorIdx] }} text-xs space-y-0.5">
                                ...
                                <div class="text-[10px] opacity-60 truncate">{{ $entry->openedCourse->grade->name_th ?? '' }} / {{ $entry->openedCourse->classroom->name ?? '' }}</div>
                            </div>
                        </td>
                        @else
