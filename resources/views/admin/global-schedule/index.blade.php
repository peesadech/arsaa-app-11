...
<div class="min-h-screen bg-gray-50 dark:bg-[#18191a] py-10 px-4 sm:px-6 lg:px-8 transition-colors duration-300">
    <div class="max-w-6xl mx-auto">
        ...
        <div class="flex items-center space-x-4 mb-8">
            ...
            <div>
                <h1 class="text-2xl font-extrabold text-gray-900 dark:text-white tracking-tight">กำหนดการเรียนการสอน</h1>
                <p class="text-sm text-gray-500 dark:text-gray-400 font-medium px-1">Global Schedule Configuration</p>
            </div>
        </div>
        ...
        <form action="{{ route('admin.global-schedule.update') }}" method="POST" id="scheduleForm">
            @csrf @method('PUT')
...
function renderTable() {
    const days = state.teaching_days; // ALL_DAYS always
    const dur  = Number(document.getElementById('period_duration').value) || 50;
    ...
    for (let p = 1; p <= maxP; p++) {
        ...
        // Break row (between periods, not after the last)
        if (p < maxP) {
