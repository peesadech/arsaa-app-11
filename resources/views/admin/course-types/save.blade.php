        ...
        <div class="bg-white dark:bg-[#242526] rounded-[2.5rem] shadow-xl shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-[#3a3b3c] overflow-hidden transform transition-all">
            ...
            <div class="p-8 sm:p-10">
                ...
                <form action="{{ $actionUrl }}" method="POST" class="space-y-6" id="courseTypeForm">
                    ...
                    <!-- Grading Scheme (default ของประเภทวิชานี้) -->
                    <div class="space-y-2">
                        <label for="grading_scheme_id" class="block text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest px-1">
                            {{ __('Grading Scheme') }} <span class="normal-case font-normal text-gray-400">({{ __('default for courses of this type') }})</span>
                        </label>
                        <div class="group relative">
                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-gray-400 {{ $focusText }} transition-colors">
                            ...
                            >
                                ...
                                @foreach($gradingSchemes as $scheme)
                                    <option value="{{ $scheme->id }}" {{ old('grading_scheme_id', $isEdit ? $courseType->grading_scheme_id : '') == $scheme->id ? 'selected' : '' }}>
                                        {{ $scheme->name }}
                                    </option>
                                @endforeach
                            </select>
