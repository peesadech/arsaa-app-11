<x-layouts.admin :header="__('Course Types')" :subheader="__('Manage course types like theory, practice, lab, activity')">
    <x-slot name="actions">
        <x-button icon="plus" :href="route('admin.course-types.create')">{{ __('New Course Type') }}</x-button>
    </x-slot>

    <div x-data="{ deleteTarget: null }" x-on:open-delete.window="deleteTarget = $event.detail">
        <x-card padded="false">
            <x-data-table
                :endpoint="route('admin.course-types.index')"
                :columns="[
            ...
            >
                ...
                <x-slot name="rows">
                    @include('admin.course-types._rows', ['courseTypes' => $courseTypes])
                </x-slot>
            </x-data-table>
        </x-card>
        ...
        <div x-show="deleteTarget !== null" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" x-transition.opacity>
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
                ...
                <div class="mt-6 flex justify-end gap-2">
                    ...
                    <form method="POST" x-bind:action="`{{ url('admin/course-types') }}/${deleteTarget?.id}`">
                        @csrf @method('DELETE')
