<x-layouts.admin :header="__('Education Levels')" :subheader="__('Manage education levels')">
    <x-slot name="actions">
        <x-button icon="plus" :href="route('admin.education-levels.create')">{{ __('New Education Level') }}</x-button>
    </x-slot>

    <div x-data="{ deleteTarget: null }" x-on:open-delete.window="deleteTarget = $event.detail">
        <x-card padded="false">
            <x-data-table
                :endpoint="route('admin.education-levels.index')"
                :columns="[
            ...
            >
                ...
                <x-slot name="rows">
                    @include('admin.education-levels._rows', ['educationLevels' => $educationLevels])
                </x-slot>
            </x-data-table>
        </x-card>
        ...
        <div x-show="deleteTarget !== null" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" x-transition.opacity>
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
                ...
                <div class="mt-6 flex justify-end gap-2">
                    ...
                    <form method="POST" x-bind:action="`{{ url('admin/education-levels') }}/${deleteTarget?.id}`">
                        @csrf @method('DELETE')
