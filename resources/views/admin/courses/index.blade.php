<x-layouts.admin :header="__('Course Management')" :subheader="__('Manage system courses')">
    <x-slot name="actions">
        <x-button icon="plus" :href="route('admin.courses.create')">{{ __('New Course') }}</x-button>
    </x-slot>

    <div x-data="{ deleteTarget: null }" x-on:open-delete.window="deleteTarget = $event.detail">
        <x-card padded="false">
            <x-data-table
                :endpoint="route('admin.courses.index')"
                :columns="[
                    ['key' => 'name', 'label' => __('Course Name'), 'sortable' => true],
                    ['key' => null, 'label' => __('Subject Group'), 'sortable' => false],
                    ['key' => null, 'label' => __('Grade'), 'sortable' => false],
                    ['key' => null, 'label' => __('Semester'), 'sortable' => false],
                    ['key' => null, 'label' => __('Grading Scheme'), 'sortable' => false],
            ...
            >
                <x-slot name="filters">
                    <select x-model="state.subject_group_id" @change="changeFilter()"
                    ...
                    <label class="relative block w-full sm:w-64">
                        ...
                        <input type="search" x-model="state.search" @input.debounce.350ms="changeFilter()"
                               placeholder="{{ __('Search courses...') }}" class="form-input pl-9 rounded-lg w-full">
                    </label>
                </x-slot>

                <x-slot name="rows">
                    @include('admin.courses._rows', ['courses' => $courses])
                </x-slot>
            </x-data-table>
        </x-card>
        ...
        <div x-show="deleteTarget !== null" x-cloak class="fixed inset-0 z-50 flex items-center justify-center p-4" x-transition.opacity>
            ...
            <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
                ...
                <div class="mt-6 flex justify-end gap-2">
                    ...
                    <form method="POST" x-bind:action="`{{ url('admin/courses') }}/${deleteTarget?.id}`">
                        @csrf @method('DELETE')
