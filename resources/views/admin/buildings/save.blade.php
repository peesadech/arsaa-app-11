...
@php
    $isEdit = isset($building);
    $actionUrl = $isEdit ? route('admin.buildings.update', $building->id) : route('admin.buildings.store');

    $title = $isEdit ? __('Edit Building') : __('Create New Building');
    $subtitle = $isEdit ? __('Update building details') : __('Building Registration');
    ...
    $cardTitle = $isEdit ? __('Modify Building') : __('Building Details');
    $cardDesc = $isEdit
        ? __('You are updating building #:id. Ensure all details are correct.', ['id' => $building->id])
        : __('Define a new building and its descriptions.');
    ...
    $btnText = $isEdit ? __('Save Changes') : __('Create Building');
    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-white dark:bg-[#242526] rounded-[2.5rem] shadow-xl shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-[#3a3b3c] overflow-hidden transform transition-all">
            ...
            <div class="p-8 sm:p-10">
                ...
                <!-- Form -->
                <form action="{{ $actionUrl }}" method="POST" class="space-y-6" id="buildingForm">
                    @csrf
                    @if($isEdit)
                        @method('PUT')
                    @endif
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="group relative">
                                ...
                                <input
                                    ...
                                    placeholder="{{ __('e.g. อาคาร 1') }}"
                                    value="{{ old('name_th', $isEdit ? $building->name_th : '') }}"
                                    required
                                />
                            </div>
                            @error('name_th')
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="group relative">
                                ...
                                <input
                                    ...
                                    value="{{ old('name_en', $isEdit ? $building->name_en : '') }}"
                                    required
                                />
                            </div>
                            @error('name_en')
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="group relative">
                            ...
                            >{{ old('description', $isEdit ? $building->description : '') }}</textarea>
                        </div>
                        @error('description')
                            <p class="text-[10px] font-bold text-rose-500 mt-1 px-1">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            <label class="relative group cursor-pointer">
                                <input type="radio" name="status" value="1" class="peer hidden" {{ old('status', $isEdit ? $building->status : 1) == 1 ? 'checked' : '' }}>
                            ...
                            <!-- Not Active Status -->
                            <label class="relative group cursor-pointer">
                                <input type="radio" name="status" value="2" class="peer hidden" {{ old('status', $isEdit ? $building->status : 1) == 2 ? 'checked' : '' }}>
